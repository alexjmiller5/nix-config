...
Both files are generated by `just manual` from `manual.steps` entries declared
next to the module that needs each step (`modules/manual-steps.nix`); on a
machine, `manual-check` reports which verifiable steps are still owed.
